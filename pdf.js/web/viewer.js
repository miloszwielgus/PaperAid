...
import { state } from './paper-aid/state.js';
import { createTooltip } from './paper-aid/tooltip.js';
import { getWordWithContext } from './paper-aid/cursorUtils.js';
import { matchWordAndTerm } from './paper-aid/stringUtils.mjs';
import { loadDefinitionsForTab } from './paper-aid/indexeddb.js';
...
document.addEventListener('mousemove', event => {
  state.currentWord = getWordWithContext(event, state.context);
...
// Event listener for keydown (to show tooltip)
document.addEventListener('keydown', event => {
  if (event.key === 'Control' && state.currentWord) {
    if (state.definitions) {
      const { bestTerm, bestDefinition } = matchWordAndTerm(
        state.currentWord,
        state.definitions,
        state.context
      );
      if (bestTerm && bestDefinition) {
        tooltip.innerText = bestTerm + ': ' + bestDefinition;
        tooltip.style.display = 'block';
      } else {
        tooltip.innerText = 'No definitions available for this word/phrase.';
        tooltip.style.display = 'block';
      }
    } else {
      tooltip.innerText = 'No definitions available for this PDF';
      tooltip.style.display = 'block';
    }
  }
});
...
window.addEventListener('DOMContentLoaded', () => {
  loadDefinitionsForTab();
});

chrome.runtime.onMessage.addListener((message, sender) => {
  if (message.type === 'NEW_DEFINITIONS') {
    loadDefinitionsForTab();
  }
});
...
chrome.runtime.onMessage.addListener((message, sender) => {
  if (
    message.type === 'FAILED_EXTRACTING' &&
    ...
    chrome.notifications.create({
      ...
      title: 'Paper Aid',
      message:
        'Failed to extract definitions from the selected text. Please try again.',
    });
  }
});
